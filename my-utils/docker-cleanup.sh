...
docker ps --format "{{.ID}} {{.Names}}" -af status=exited | egrep -v '\-code|rpm\-repo' | awk '{print $1}' | xargs -n1 -r docker inspect -f '{{.Id}} {{.Name}} {{.State.FinishedAt}}' | awk '$3 <= "'$(date -d '7 days ago' -Ins --utc | sed 's/+0000/Z/')'" {print $1}' | xargs -r docker rm > /dev/null 2>&1
