...
#		--setopt=group_package_types=mandatory -y groupinstall "${install_groups[@]}"
        yum -c "$yum_config" --installroot="$target" --releasever=/ --setopt=tsflags=nodocs \
                ...
                --disablerepo kylinmips --disablerepo openstack\* --disablerepo neokylin\* \
                install \
                basesystem bash coreutils curl filesystem glibc hostname iputils less license-client-libs ncurses nklicadm passwd procps-ng rootfiles rpm setup shadow-utils systemd tar util-linux vim-minimal yum
fi
...
yum -c "$yum_config" --installroot="$target" -y clean all || true
rm -rf "$target"/var/cache/yum || true
